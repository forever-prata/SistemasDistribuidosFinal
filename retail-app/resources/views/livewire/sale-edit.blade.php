<div class="p-4 min-h-screen">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-white">Editar Venda #{{ $saleId }}</h2>
        ...
        <form wire:submit.prevent="update" class="space-y-6">
            ...
            <div>
                ...
                <select wire:model="customer_id" class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    @foreach ($customers as $customer)
                        <option value="{{ $customer->_id }}" {{ $customer->_id == $customer_id ? 'selected' : '' }}>
                            {{ $customer->name }}
                        </option>
                    @endforeach
                </select>
                        ...
                        <select wire:model="items.{{ $index }}.product_id"
                            ...
                            @foreach($products as $product)
                                <option value="{{ $product->_id }}"
                                        {{ $product->_id == $item['product_id'] ? 'selected' : '' }}
            ...
            <div class="flex justify-end pt-4">
                <button type="submit"
                    ...
                    Atualizar Venda (R$ {{ number_format($total, 2, ',', '.') }})
                </button>
            </div>
        </form>
    </div>
